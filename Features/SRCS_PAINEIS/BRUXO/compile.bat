...
echo LOCAL_MODULE := bruxo >> "%JNI_DIR%\Android.mk"
...
$(LOCAL_PATH)/../../dobby ^
$(LOCAL_PATH)/../.. >> "%JNI_DIR%\Android.mk"

echo LOCAL_LDFLAGS += -llog -landroid -lEGL -lGLESv2 -lGLESv3 >> "%JNI_DIR%\Android.mk"
echo LOCAL_CPPFLAGS += -std=c++17 -fexceptions -frtti -Wno-error -Wno-format-security -Wno-c++11-narrowing -Wno-ignored-attributes -DkNO_KEYSTONE=1 >> "%JNI_DIR%\Android.mk"
...
if errorlevel 1 (
  echo [ERRO] Falha na compilacao do painel BRUXO.
  rem rmdir /s /q "%OUTPUT_DIR%" 2>nul
  pause
  exit /b 1
)

if exist "%OUTPUT_DIR%\libs\%TARGET_ARCH%\libbruxo.so" (
  move /y "%OUTPUT_DIR%\libs\%TARGET_ARCH%\libbruxo.so" "%OUTPUT_DIR%\bruxo.so" >nul
  echo [OK] Biblioteca compilada: %OUTPUT_DIR%\bruxo.so
) else (
  echo [ERRO] libbruxo.so nao encontrada
)

rmdir /s /q "%OUTPUT_DIR%\obj"
rmdir /s /q "%OUTPUT_DIR%\libs"
rmdir /s /q "%JNI_DIR%"
echo [INFO] Compilacao BRUXO concluida.
pause
exit /b 0
