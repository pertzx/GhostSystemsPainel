...
ImVec4 wallColor = ImVec4(1.0f, 0.0f, 0.0f, 1.0f); 
...
const char *isDumpLocation = ("/storage/emulated/0/Download/Shaders.txt");
...
GLint new_glGetUniformLocation(GLuint program, const GLchar *name) {
    ...
    for (const auto& shaderNameInVector : shaderNames) {
        if (shaderNameInVector == name) {
            alreadyExists = true;
            break;
        }
    }
