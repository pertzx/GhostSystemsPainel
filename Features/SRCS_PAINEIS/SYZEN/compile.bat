...
echo LOCAL_MODULE := syzen >> "%JNI_DIR%\Android.mk"
...
$(LOCAL_PATH)/../../Login ^
$(LOCAL_PATH)/../../eLogin >> "%JNI_DIR%\Android.mk"
...
if errorlevel 1 (
  echo [ERRO] Falha na compilacao do painel SYZEN.
  rmdir /s /q "%OUTPUT_DIR%" 2>nul
  pause
  exit /b 1
)

if exist "%OUTPUT_DIR%\libs\%TARGET_ARCH%\libsyzen.so" (
  move /y "%OUTPUT_DIR%\libs\%TARGET_ARCH%\libsyzen.so" "%OUTPUT_DIR%\syzen.so" >nul
  echo [OK] Biblioteca compilada: %OUTPUT_DIR%\syzen.so
) else (
  echo [ERRO] libsyzen.so nao encontrada
)

rmdir /s /q "%OUTPUT_DIR%\obj"
rmdir /s /q "%OUTPUT_DIR%\libs"
rmdir /s /q "%JNI_DIR%"
echo [INFO] Compilacao SYZEN concluida.
pause
exit /b 0
