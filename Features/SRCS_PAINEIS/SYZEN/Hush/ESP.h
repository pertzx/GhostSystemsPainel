...
void DrawBones(const std::vector<BonePos>& bones, ImDrawList* drawList, float distance, float screenHeight)
{
    ...
    const ImU32 boneColor     = IM_COL32(255, 0, 0, 255); // VERMELHO PURO E VIVO 🔥
...
void DrawESP(float screenWidth, float screenHeight) {
    ...
    if (Inimigos) {
        if (numVisibleEnemies > 1) {
            int enemiesToShowOnTop = std::min(numVisibleEnemies - 1, 60);
            const char* inimigos = OBFUSCATE("");
            std::string topText = std::to_string(enemiesToShowOnTop) + inimigos;
            float textHeight = 40.0f;
            ImVec2 topTextSize = ImGui::CalcTextSize(topText.c_str(), NULL, false);
            ImVec2 topTextPos(screenWidth / 2.0f - topTextSize.x / 2.0f, 10);
            float backgroundPadding = 20.0f;
            ImVec2 rectMin(topTextPos.x - backgroundPadding, topTextPos.y - 10);
            ImVec2 rectMax(topTextPos.x + topTextSize.x + backgroundPadding, topTextPos.y + textHeight + 10);
            
            draw->AddRectFilled(rectMin, rectMax, ImColor(0, 0, 0, 0), 10.0f);
            draw->AddRect(rectMin, rectMax, ImColor(0, 0, 0, 0), 2.0f);
            draw->AddText(NULL, textHeight, topTextPos, ImColor(255, 255, 255, 255), topText.c_str());
        }
