...
if (FindURL.find(SO("https://4m.xiters.site/mod/")) != std::string::npos && FindParams.find(SO("https://4m.xiters.site/mod/")) != std::string::npos)
{
if(Request != nullptr)
{
...
ImGui::End();
    ...
    const ImVec4 purple_custom = ImVec4(0.608f, 0.027f, 0.922f, 1.0f
);
...
if (showLoginScreen) {
            if (ImGui::Begin(OBFUSCATE("Login BX"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse)) {
            
            if (loginn) {
            ImGui::CenteredText(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), O("Logging in..."));
            } else {
            
            ImGui::InputTextWithHint("##user", "Usuario", Username, 128, ImGuiInputTextFlags_CharsNoBlank);
            ...
            ImGui::InputTextWithHint("##pw", "Key", Password, 128, ImGuiInputTextFlags_CharsNoBlank);
            if (ImGui::Button(O("Colar key"), {ImGui::GetContentRegionAvail().x , 0})) {
